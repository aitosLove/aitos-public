...
import { TokenOnPortfolio, OnChainCoin } from "../../type";
import { AptosAccountManager } from "./account";
...
interface GetHoldingParams {
  select_portfolio: OnChainCoin[];
}
export async function getHolding_apt(
  account: AptosAccountManager,
  { select_portfolio }: GetHoldingParams
) {
  const aptosClient = account.aptosClient;
  const accountAddress = account.account.accountAddress.toString();
  try {
    // 获取用户持有的代币信息
    ...
    // 处理持仓数据，过滤并计算百分比等
    const coinHolding = await processHoldings({
