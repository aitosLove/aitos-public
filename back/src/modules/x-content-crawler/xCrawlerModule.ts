...
export class XCrawlerModule {
   ...
   */
  public async initialize(userId: string, cookies: any[]): Promise<boolean> {
    this.userId = userId;
    ...
    try {
      ...
      // Initialize crawler
      const initialized = await this.crawler.initialize(userId,cookies);
      ...
      try {
        this.followings = await getFollowings(userId);
  ...
  public async updateFollowingList(): Promise<Following[]> {
    ...
    try {
      ...
      await saveFollowings(this.userId, this.followings);
