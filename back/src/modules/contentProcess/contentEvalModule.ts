...
} from "../xContentCrawler/types";
import { ContentEvaluator } from "./contentEvaluator";
import { savePerplexitySearchResult, saveContentInsight } from "./ContentDbOp";
...
export class ContentEvaluationManager {
  ...
  private async handleContentToProcess(event: AgentEvent): Promise<void> {
    try {
      ...
      // Evaluate the post
      const contentInsight = await Evaluator.evaluatePost(post, this.agentId);

      if (contentInsight) {
        // Save content insight to database
        try {
          const savedInsightId = await saveContentInsight(contentInsight, this.agentId);
          console.log(`Saved content insight with ID: ${savedInsightId}`);
        } catch (error) {
          console.error("Error saving content insight:", error);
        }
