...
export function getMarketInsightPrompt_BSC({
  analysis_portfolio,
  preference_instruct,
}: {
  ...
  Here are a few crypto asset ratio data for one month. Please analyze it. What do you think of the current market situation? And what's your opinion of BNB and the Binance Smart Chain ecosystem?
  
  [Prefenence Instruct]
  ${preference_instruct}
  
  [Potential Assets Indicators Interpretation] & [All these tokens are BSC ecosystem tokens]
  ${analysis_portfolio
    .map(
      (coin) =>
...
export function getTradingPrompt_BSC({
  current_holding,
  token_portfolio,
  preference_instruct,
}: {
  ...
  const trading_instruct = `You are a professional trader and you have a portfolio on Binance Smart Chain, which includes BNB, BUSD, USDT and other potential altcoins. You want to adjust the portfolio to get best profit according to market situation. You main method is to adjust the weight of each token holding. 
...
export function getDefiInsightPrompt_BSC({
  preference_instruct,
}: {
  preference_instruct: string;
}) {
  const defi_instruct = `
  You are an expert in decentralized finance (DeFi) strategies. Your task is to formulate a DeFi strategy tailored to the user's current holdings using the provided information about various DeFi protocol pools on Binance Smart Chain. You will receive:
  {
  ...
  - Exposure Management:
  Exposure refers to how an asset's price movement impacts the portfolio's balance. Holding volatile assets like altcoins, BTC, or ETH without hedging creates exposure to their price fluctuations. For a USD-based portfolio, holding stablecoins (e.g., BUSD, USDT, USDC) does not create exposure due to their pegged value. Minimize unnecessary exposure unless the user already holds the asset and accepts that risk.
  
  - Security:
  Prioritize protocols with audited smart contracts and a strong security track record to reduce the risk of exploits or losses. BSC has many protocols, but security levels can vary significantly.
  ...
  - If the User Holds Stablecoins (e.g., BUSD, USDT, USDC):
  ...
  - If the User Holds Altcoins, BNB, BTC, or ETH directly:
  ...
  - If the User Holds Major Assets (e.g., USD, BNB, ETH, BTC) and Wants Yields of Altcoin Without holding Altcoin Exposure:
  ...
  Consider the unique aspects of BSC such as its lower gas fees, high transaction throughput, and the role of BNB in the ecosystem when formulating your recommendations.
  `;
