...
import { savePost, saveUserCookies, updateFollowingLastUpdated } from "./dbOp";
...
export class XCrawler {
  ...
  public async fetchCookies(): Promise<any[]> {
    ...
    try {
      ...
      // Save cookies with agentId if we have user information
      if (pageCookies.length > 0 && this.userId && this.agentId) {
        await saveUserCookies(this.userId, this.agentId, pageCookies);
      }
  ...
  public async getFollowings(): Promise<Following[]> {
    ...
    try {
      ...
      if (cookies.length > 0 && this.userId) {
        await saveUserCookies(this.userId, this.agentId, cookies);
      }
  ...
  public async processSingleFollowing(
    ...
    try {
      ...
      await updateFollowingLastUpdated(this.userId, this.agentId, following.username);
  ...
  private async collectPosts(username: string): Promise<XPost[]> {
    ...
    for (const post of allPosts) {
      try {
        await savePost(post, this.agentId);
      } catch (dbError) {
