...
export function getMarketInsightPrompt_SOL({
  analysis_portfolio,
  preference_instruct,
}: {
  ...
  Here are a few crypto asset ratio data for one month. Please analyze it. What do you think of the current market situation? And what's your opinion of SOL and the Solana ecosystem?
  
  [Preference Instruct]
  ${preference_instruct}
  
  [Potential Assets Indicators Interpretation] & [All these tokens are Solana ecosystem tokens]
  ${analysis_portfolio
    .map(
      (coin) =>
...
export function getTradingPrompt_SOL({
  current_holding,
  token_portfolio,
  preference_instruct,
}: {
  ...
  const trading_instruct = `You are a professional trader and you have a portfolio on Solana, which includes SOL, USDC and other potential altcoins. You want to adjust the portfolio to get best profit according to market situation. Your main method is to adjust the weight of each token holding. 
...
export function getDefiInsightPrompt_SOL({
  preference_instruct,
}: {
  preference_instruct: string;
}) {
  const defi_instruct = `
  You are an expert in decentralized finance (DeFi) strategies. Your task is to formulate a DeFi strategy tailored to the user's current holdings using the provided information about various DeFi protocol pools on Solana. You will receive:
  {
  ...
  - Security:
  Prioritize protocols with audited smart contracts and a strong security track record to reduce the risk of exploits or losses. Solana has many protocols, but security levels can vary significantly.
  ...
  - If the User Holds Altcoins, SOL, BTC, or ETH directly:
  ...
  - If the User Holds Major Assets (e.g., USD, SOL, ETH, BTC) and Wants Yields of Altcoin Without holding Altcoin Exposure:
  ...
  Consider the unique aspects of Solana such as its ultra-fast transaction speed, low gas fees, and the role of SOL in staking and governance when formulating your recommendations.
  `;
