...
export function getTradingPrompt_SUI({
  current_holding,
  token_portfolio,
  preference_instruct,
}: {
  ...
  const trading_instruct = `You are a professional trader and you have a portfolio on SUI blockchain, which includes SUI, USDC and other potential altcoins. You want to adjust the portfolio to get best profit according to market situation. You main method is to adjust the weight of each token holding. 
...
export function getDefiInsightPrompt_SUI({
  preference_instruct,
}: {
  preference_instruct: string;
}) {
