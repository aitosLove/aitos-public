...
export async function saveContentInsight(
  ...
  try {
    ...
    if (existingInsight) {
      ...
      console.log(
        `[DB] Updated content insight for ${insight.username} at ${insight.source}`
      );
      ...
      console.log(
        `[DB] Created new content insight for ${insight.username} at ${insight.source}`
      );
...
export async function savePerplexitySearchResult(
  ...
  try {
    ...
    console.log(
      `[DB] Saved Perplexity search result with ID: ${perplexitySearchId} (${searchResult.citations.length} citations)`
    );
...
export async function getPerplexitySearchesBySource(
  ...
  try {
    ...
    console.log(
      `[DB] Retrieved ${results.length} Perplexity searches for source: ${sourceUrl}`
    );
    return results;
  } catch (error) {
    console.error(
